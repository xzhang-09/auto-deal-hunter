...
class App:
    ...
    def run(self):
        with gr.Blocks(title="auto-deal-hunter", fill_width=True) as ui:
            log_data = gr.State([])
            ...
            with gr.Row():
                gr.Markdown(
                    '<div style="text-align: center;font-size:24px"><strong>auto-deal-hunter</strong> - MCP-based Autonomous Deal Agent</div>'
                )
            with gr.Row():
                gr.Markdown(
                    '<div style="text-align: center;font-size:14px">LLM + MCP tools (scan, estimate, notify) collaborate to find and alert on deals.</div>'
                )
            with gr.Row():
                opportunities_dataframe = gr.Dataframe(
                    headers=["Deals", "Price", "Estimate", "Discount", "URL"],
                    wrap=True,
